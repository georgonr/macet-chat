...
afterEvaluate {
  ...
  tasks.create("adjustFormatting") {
    doLast {
      ...
      // base is the only strings file left, so the cross-check below has nothing to compare it
      // against - what still runs is the CDATA normalisation and the format-specifier validation
      // of base itself. The lv/strings.xml exclusion that used to sit here went with the
      // translations.
